...
from .selecionar_diarista_diaria_service import selecionar_diarista_diaria


def relacionar_candidato_diaria(diaria_id, diarista_id):
...
def contratar_diarista_diaria(diaria, diarista_id):
    if diaria.status != 2:
        raise serializers.ValidationError("Apenas diarias pagas podem ser cadidatadas")
    diarista = Usuario.objects.get(id=diarista_id)
